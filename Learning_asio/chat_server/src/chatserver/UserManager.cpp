...
bool UserManager::loadUsersFromDb()
{
    ...
    {
        LOG_FATAL("UserManager::LoadUsersFromDb failed, please check params: dbserver: %s, dbusername: %s, , dbpassword: %s, dbname: %s",
    ...
    while (true)
    {
        ...
        LOG_INFO("userid: %d, username: %s, password: %s, nickname: %s, signature: %s", u.userid, u.username.c_str(), u.password.c_str(), u.nickname.c_str(), u.signature.c_str());
...
bool UserManager::addUser(User& u)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName))
    {
        LOG_ERROR("UserManager::AddUser failed, please check params: dbserver: %s, , dbusername: %s, dbpassword: %s, dbname: %s", m_strDbServer.c_str(), m_strDbUserName.c_str(), m_strDbPassword.c_str(), m_strDbName.c_str());
        return false;
    }
    ...
    {
        LOG_WARNING("insert user error, sql: %s", sql);
        return false;
    }
    //设置一些字段的默认值
...
bool UserManager::makeFriendRelationshipInDB(int32_t smallUserid, int32_t greaterUserid)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName))
    {
        LOG_ERROR("UserManager::LoadUsersFromDb failed, please check params: dbserver: %s, dbusername: %s, dbpassword: %s, , dbname: %s",
            m_strDbServer.c_str(), m_strDbUserName.c_str(), m_strDbPassword.c_str(), m_strDbName.c_str());
        return false;
    }
    ...
    if (!pConn->execute(sql))
    {
        LOG_ERROR("make relationship error, sql: %s, smallUserid: %s, greaterUserid: %s", sql, smallUserid, greaterUserid);
        return false;
    }
...
bool UserManager::releaseFriendRelationshipInDBAndMemory(int32_t smallUserid, int32_t greaterUserid)
{
    ...
    if (!pConn->execute(sql))
    {
        LOG_ERROR("release relationship error, sql: %s, smallUserid: %d, , greaterUserid: %d", sql, smallUserid, greaterUserid);
        return false;
    }
...
bool UserManager::updateUserInfoInDb(int32_t userid, const User& newuserinfo)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName))
    {
        LOG_ERROR("UserManager::Initialize db failed,  please check params: dbserver: %s, dbusername: %s, dbpassword: %s, dbname: %s",
            m_strDbServer.c_str(), m_strDbUserName.c_str(), m_strDbPassword.c_str(), m_strDbName.c_str());
        return false;
    }
    ...
    LOG_ERROR("Failed to update userinfo to db, find exsit user in memory error, m_allCachedUsers.size(): %d, userid: %d, sql: %s", m_allCachedUsers.size(), userid, osSql.str().c_str());
...
bool UserManager::modifyUserPassword(int32_t userid, const std::string& newpassword)
{
    ...
    LOG_ERROR("Failed to update user password to db, find no exsit user in memory error, m_allCachedUsers.size(): %d, userid: %d, sql: %s", m_allCachedUsers.size(), userid, osSql.str().c_str());
...
bool UserManager::updateUserTeamInfoInDbAndMemory(int32_t userid, const std::string& newteaminfo)
{
    ...
    LOG_ERROR("Failed to update user teaminfo to db, find no exsit user in memory error, m_allCachedUsers.size(): %d, userid: %d, sql: %s", m_allCachedUsers.size(), userid, osSql.str().c_str());
...
bool UserManager::deleteTeam(int32_t userid, const std::string& deletedteamname)
{
    ...
    if (!pConn->execute(osSql.str().c_str()))
    {
        LOG_ERROR("DeleteTeam error, sql: %s, userid: %d, deletedteamname: %s", osSql.str().c_str(), userid, deletedteamname.c_str());
        return false;
    }
...
bool UserManager::modifyTeamName(int32_t userid, const std::string& newteamname, const std::string& oldteamname)
{
    ...
    if (!pConn->execute(osSql.str().c_str()))
    {
        LOG_ERROR("ModifyTeamName error, sql: %s, userid: %d, newteamname: %s, oldteamname: %s", osSql.str().c_str(), userid, newteamname.c_str(), oldteamname.c_str());
        return false;
    }
...
bool UserManager::moveFriendToOtherTeam(int32_t userid, int32_t friendid, const std::string& newteamname)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName))
    {
        LOG_ERROR("UserManager::Initialize db failed, please check params: dbserver: %s, dbusername: %s, dbpassword: %s, dbname: %s, userid: %d, friendid: %d, newteamname: %s",
    ...
    if (!pConn->execute(osSql.str().c_str()))
    {
        LOG_ERROR("MoveFriendToOtherTeam, sql: %s, userid: %d, friendid: %d, newteamname: %s", osSql.str().c_str(), userid, friendid, newteamname.c_str());
        return false;
    }

    LOG_INFO("MoveFriendToOtherTeam db operation successfully, userid: %d, friendid: %d, sql: %s", userid, friendid, osSql.str().c_str());
...
bool UserManager::addGroup(const char* groupname, int32_t ownerid, int32_t& groupid)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName))
    {
        LOG_ERROR("UserManager::AddGroup failed, please check params: dbserver: %s, dbusername: %s, dbname: %s, dbpassword: %s, groupname: %s, ownerid: %d",
...
bool UserManager::saveChatMsgToDb(int32_t senderid, int32_t targetid, const std::string& chatmsg)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName))
    {
        LOG_ERROR("UserManager::SaveChatMsgToDb failed, please check params: dbserver: %s, dbusername: %s, dbpassword: %s, dbname: %s", m_strDbServer.c_str(), m_strDbUserName.c_str(), m_strDbPassword.c_str(), m_strDbName.c_str());
        return false;
    }
    ...
    {
        LOG_ERROR("UserManager::SaveChatMsgToDb, sql: %s, senderid: %d, targetid: %d, chatmsg: %s", sql.str().c_str(), senderid, targetid, chatmsg.c_str());
        return false;
    }
